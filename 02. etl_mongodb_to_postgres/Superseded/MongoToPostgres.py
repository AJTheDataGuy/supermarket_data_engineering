...
def main():
    ...
    flattened_json_df_w_specials.to_sql(
...
def connect_to_mongodb(
    ...
    Returns:
    1. Connection a specific MongoDB collection
    """
...
def connect_to_postgresql(
    ...
    Returns:
    1. sqlalchemy engine to use to connect to PostgreSQL
    """
...
def define_column_data_types_for_sql() -> dict:
    ...
    Returns:
    1. Dictionary of how certain (but not all) columns present in the MongoDB
    JSON data should be represented in the PostgreSQL data
    """
    data_type_definitions_dict = {
        "id": sqlalchemy.types.INTEGER(),
        "date_extracted": sqlalchemy.types.DATE(),
        "featured": sqlalchemy.types.BOOLEAN(),
        "restrictions.retaiLimit": sqlalchemy.types.INTEGER(),
        "restrictions.promotionalLimit": sqlalchemy.types.INTEGER(),
        "restrictions.liquorAgeRestrictionFlag": sqlalchemy.types.BOOLEAN(),
        "restrictions.tobaccoAgeRestrictionFlag": sqlalchemy.types.BOOLEAN(),
        "restrictions.restrictedByOrganisation": sqlalchemy.types.BOOLEAN(),
        "pricing.now": sqlalchemy.types.NUMERIC(),
        "pricing.was": sqlalchemy.types.NUMERIC(),
        "pricing.unit.quantity": sqlalchemy.types.INTEGER(),
        "pricing.unit.ofMeasureQuantity": sqlalchemy.types.INTEGER(),
        "pricing.unit.price": sqlalchemy.types.NUMERIC(),
        "pricing.unit.isWeighted": sqlalchemy.types.BOOLEAN(),
        "pricing.onlineSpecial": sqlalchemy.types.BOOLEAN(),
        "pricing.multiBuyPromotion.id": sqlalchemy.types.INTEGER(),
        "pricing.multiBuyPromotion.minQuantity": sqlalchemy.types.INTEGER(),
        "pricing.multiBuyPromotion.reward": sqlalchemy.types.NUMERIC(),
    }
...
def add_specials_columns_if_not_present(
    ...
    Parameters:
    1. The flattened mongodb dataframe that may or may not contain the
        pricing specials columns

    Returns:
    1. MongoDB flattened data with specials columns in the dataframe
    """
    expected_specials_columns_set = {
        ...
        "pricing.multiBuyPromotion.type" "pricing.multiBuyPromotion.id",
        "pricing.multiBuyPromotion.minQuantity" "pricing.multiBuyPromotion.reward",
...
def test_if_schema_has_changed(raw_mongo_db_df: pd.DataFrame):
    ...
    2. raw_mong_db_df: dataframe with data from MongoDB.

    Returns:
    1. Boolean True / False on whether the schema has changed.
...
def test_schema_change_if_deprecated_columns(
    ...
    Returns:
    1. Boolean True / False if deprecated columns are present.
...
def get_expected_postgres_columns_list() -> list:
    ...
    Returns:
    1. List of expected columns in the PostgreSQL target table
