...
def main():
    webpage = "https://www.coles.com.au/browse/fruit-vegetables?page=2"
    raw = extract_single_webpage_text(webpage)
    json_regex = compile_regex_for_initial_json_extraction()
    found_json_text = re.findall(json_regex,raw)
    print(found_json_text[1])
    today = str(date.today())

    errors_list = []
    json_list = []
    for json_string in found_json_text:
        try:
